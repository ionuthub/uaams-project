...
| PRD-REG-02 | Registration captures nationality. | Removed because the value was discarded. | Missing | Approve a profile/application location, add schema/rules, restore the field and test persistence. | Silvana, Dawid and Alina | Schema PR, field screenshot and stored-record test |
...
| PRD-REG-04 | Registration records privacy-policy acceptance. | Removed because no real policy page or consent record exists. | Missing | Obtain approved privacy wording, add policy/version/timestamp model and accessible consent UI. | Ionut coordinates; Silvana/Dawid/Alina deliver | Policy page, consent record and test |
...
| PRD-DOC-01 | Upload passport copy, transcripts, certificates and optional English test. | Current design permits one generic document path. | Deferred/material scope reduction | Implement typed multi-document support or obtain formal approval for the one-document interim variation. | Dawid and Alina; Silvana documents | Approved variation or multi-document tests |
| PRD-DOC-02 | Documents stored in Firebase Storage and metadata in Firestore. | Storage helper/path exist; metadata is one path and live evidence is incomplete. | Partial | Complete bucket/rules evidence and introduce approved document metadata records. | Dawid | Storage and Firestore evidence |
| PRD-DOC-03 | File size and format validation. | PDF/JPG/PNG and 10 MB rules exist. | Partial | Deploy and execute allowed/rejected live tests. | Dawid and test owner | Type/size pass-fail evidence |
| PRD-DOC-04 | Required evidence gates submission. | A draft can currently be submitted with no `documentPath`. | Missing | Enforce the gate in trusted backend/rules, not only UI. | Dawid; Alina handles feedback | APP-09 test |
...
| PRD-DATA-04 | `documents` with application, file type, URL/path and timestamp. | No document collection; one application path is used. | Substituted/incomplete | Adopt typed collection/subcollection or approve an equivalent alternative model. | Dawid and Silvana | Model decision and document tests |
...
| PRD-TEST-02 | Integration testing for authentication/application flow. | Test plan exists; complete execution pending. | Partial | Execute #25 on an exact deployed build. | Test and feature owners | End-to-end record |
...
| PRD-DEL-02 | Architecture, schema, rules, API/service, environment and deployment documentation. | Most documents exist; PRD mapping was absent. | Partial | Complete approvals, API/config/deployment coverage and maintain this register. | Silvana, Dawid and Ionut | Merged docs/reviews |
