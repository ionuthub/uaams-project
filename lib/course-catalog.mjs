...
//
// The seeded database has one university (Southampton Solent University,
// id "solent"), so the public screens present that institution and this
// catalogue instead of the placeholder universities from the design
// prototype. Applications still reference the university document in
// Firestore; the catalogue is presentation content for the public pages.
// Sprint 3 candidate: move this into a read-only /courses collection.
...
/** Courses for one university id; only the seeded university has a catalogue. */
export function coursesForUniversity(universityId) {
  return universityId === SOLENT.id ? SOLENT_COURSES : [];
}
