...
//
// Status lifecycle (matches Elena's five status badges):
...
export async function createApplication(studentUid, universityId, formData) {
  const ref = await addDoc(collection(db, "applications"), {
    ...
    universityId,
    status: "draft",
    form: formData,           // step 1 + step 4 fields from Elena's form skeleton
...
export async function recordDecision(applicationId, adminUid, decision, message) {
  ...
  await addDoc(collection(db, "applications", applicationId, "decisions"), {
    decision,
    message,
    decidedBy: adminUid,
  ...
  // 2. Current status on the application itself
  await updateDoc(doc(db, "applications", applicationId), {
    status: decision,
