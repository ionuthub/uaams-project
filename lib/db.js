...
import { requestApplicationReferences, withApplicationReferences } from "./application-reference-client";
...
export async function submitApplication(applicationId) {
  ...
  // Assign before submission so the confirmation and both portals use the same
  // permanent reference. Retrying reuses the number rather than allocating again.
  await requestApplicationReferences([applicationId]);
...
export async function getStudentApplications(studentUid) {
  ...
  const snap = await getDocs(q);
  return withApplicationReferences(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
}
...
export async function getApplication(applicationId) {
  ...
  const [application] = await withApplicationReferences([{ id: snap.id, ...snap.data() }]);
  return application;
}
...
/**
 * Records that an admin exported the queue to CSV: who, from which
 * university, how many rows, and under what filter/search - never the
 * exported row contents themselves. One doc per export click (payments/CSV
 * scope, issue #240). Read is scoped like every other admin-authored
 * record here; there is no update or delete because an export either
 * happened or it did not.
 */
export async function logCsvExport(adminUid, universityId, rowCount, filters) {
  const db = getDbClient();
  await addDoc(collection(db, "exportLogs"), {
    adminUid,
    universityId,
    rowCount,
    statusFilter: (filters && filters.statusFilter) || "all",
    searchActive: Boolean(filters && filters.query),
...
export async function getUniversities() {
  const db = getDbClient();
  // Ordered by name so the list is stable wherever it is shown. Without an
    // explicit order Firestore falls back to document ID order, which put
    // "portsmouth" ahead of "solent" the moment a second university was seeded.
    // That silently changed which institution sat first in the apply dropdown,
    // and with it where the end-to-end suite was submitting (#25).
